...
public class ViewerPage extends FrameLayout implements Observer {
    ...
    AsyncTaskPageRendering mInitialRenderingTask = null;
    private AsyncTaskPageRendering mRenderPageTask = null;
    ...
    private ViewElement mLastFocusedElementViewer = null;
    ...
    ActivityDigiSign activity = null;

    public ViewerPage(Context context, ActivityDigiSign activity, Page pdfPage) {
        super(context);
    ...
    private class GestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        GestureListener(ViewerPage fASPageViewer) {
            this();
        }

        public boolean onDown(MotionEvent motionEvent) {
            mScroller.forceFinished(true);
            ViewCompat.postInvalidateOnAnimation(ViewerPage.this);
            return true;
        }

        public boolean onFling(MotionEvent motionEvent, MotionEvent motionEvent2, float f, float f2) {
            if (motionEvent2.getPointerCount() > 1 || SystemClock.elapsedRealtime() - mLastZoomTime < 200L) {
                return false;
            }
            mScroller.abortAnimation();
            mScroller.fling(mScrollView.getScrollX(), mScrollView.getScrollY(), ((int) (-f)) * 2, ((int) (-f2)) * 2, 0, getMaxScrollX(), 0, getMaxScrollY());
            ViewCompat.postInvalidateOnAnimation(ViewerPage.this);
            return true;
        }
        ...
        public boolean onScroll(MotionEvent motionEvent, MotionEvent motionEvent2, float f, float f2) {
            if (mIsInterceptedScrolling && mIsFirstScrollAfterIntercept) {
                ...
                return false;
            } else if (motionEvent2.getPointerCount() > 1 || SystemClock.elapsedRealtime() - mLastZoomTime < 200L) {
                return false;
            } else {
                applyScroll(mScrollView.getScrollX() + Math.round(f), mScrollView.getScrollY() + Math.round(f2));
                return true;
            }
        }
    ...
    private class ScaleGestureListener extends SimpleOnScaleGestureListener {
        ...
        ScaleGestureListener(ViewerPage fASPageViewer) {
            this();
        }
        ...
        public void resetScale() {
            mFocus = new PointF(0.0f, 0.0f);
            mScroll = new PointF(0.0f, 0.0f);
    ...
    private void onTap(MotionEvent motionEvent, boolean z) {
        ...
        if (!getImageContentRect().contains(new RectF(x, y, (getResources().getDimension(R.dimen.element_min_width) + x) + (getResources().getDimension(R.dimen.element_horizontal_padding) * 2.0f), (getResources().getDimension(R.dimen.element_min_width) + y) + (getResources().getDimension(R.dimen.element_vertical_padding) * 2.0f)))) {
            if (UtilSign.isSignatureMenuOpen()) {
                UtilSign.dismissSignatureMenu();
            }
            removeFocus();
        } else {
            this.mTouchX = x;
    ...
    public ActivityDigiSign getDocumentViewer() {
        return (ActivityDigiSign) this.mContext;
    }
    ...
    private void initRenderingAsync() {
        if (this.mImage == null && this.mImageView.getWidth() > 0 && (this.mInitialRenderingTask == null || this.mInitialRenderingTask.getStatus() == AsyncTask.Status.FINISHED)) {
            ...
            this.mInitialRenderingTask = new AsyncTaskPageRendering(this.mContext, this.mPage, new SizeF((float) this.mImageView.getWidth(), (float) this.mImageView.getHeight()), 1.0f, false, false, new AsyncTaskPageRendering.OnPostExecuteListener() {
                public void onPostExecute(AsyncTaskPageRendering fASRenderPageAsyncTask, Bitmap bitmap) {
                    if (bitmap != null && mScaleFactor == 1.0f && fASRenderPageAsyncTask.getPage() == mPage) {
                        int visibleWindowHeight = getDocumentViewer().getVisibleWindowHeight();
                        if (visibleWindowHeight > 0) {
                            mScrollView.setLayoutParams(new LayoutParams(-1, visibleWindowHeight));
                        }
                        mInitialImageSize = fASRenderPageAsyncTask.getBitmapSize();
    ...
    private synchronized void updateImageFoScale() {
        if (this.mScaleFactor != this.mBitmapScale) {
            ...
            this.mRenderPageTask = new AsyncTaskPageRendering(this.mContext, this.mPage, new SizeF((float) this.mImageView.getWidth(), (float) this.mImageView.getHeight()), this.mScaleFactor, false, false, new AsyncTaskPageRendering.OnPostExecuteListener() {
                public void onPostExecute(AsyncTaskPageRendering fASRenderPageAsyncTask, Bitmap bitmap) {
                    if (bitmap != null && mScaleFactor == fASRenderPageAsyncTask.getScale()) {
                        setImageBitmap(bitmap);
                        mBitmapScale = mScaleFactor;
                    } else if (bitmap != null) {
                        bitmap.recycle();
                    }
    ...
    public ViewElement getLastFocusedElementViewer() {
        return this.mLastFocusedElementViewer;
    }

    public SignElementPdf createElement(SignElementPdf.PDSElementType fASElementType, File file, float f, float f2, float f3, float f4) {
        SignElementPdf fASElement = new SignElementPdf(fASElementType, file);
        //fASElement.setContent(fASElementContent);
        fASElement.setRect(mapRectToPDFCoordinates(new RectF(f, f2, f + f3, f2 + f4)));
        ViewElement addElement = addElement(fASElement);
        if (fASElementType == SignElementPdf.PDSElementType.PDSElementTypeSignature) {
    ...
    private ViewElement addElement(SignElementPdf fASElement) {
        return new ViewElement(this.mContext, this, fASElement,activity);
    }
    ...
    public void showElementPropMenu(final ViewElement fASElementViewer) {
        hideElementPropMenu();
        hideElementCreationMenu();
    ...
    public void modifyElementSignatureSize(SignElementPdf fASElement, View view, RelativeLayout relativeLayout, int i, int i2) {
        ...
        if (getImageContentRect().contains(new RectF(relativeLayout.getX(), relativeLayout.getY() - f, (relativeLayout.getX() + ((float) relativeLayout.getWidth())) + ((float) i3), relativeLayout.getY() + ((float) relativeLayout.getHeight())))) {
            if (view instanceof ViewSign) {
                ViewSign viewSign = (ViewSign) view;
                viewSign.setLayoutParams(500+ i3, view.getHeight() + i4);
//                mContainerView.setLayoutParams(new RelativeLayout.LayoutParams(relativeLayout.getWidth() + i3, relativeLayout.getHeight() + i4));

                relativeLayout2.setLayoutParams(new RelativeLayout.LayoutParams(relativeLayout.getWidth() + i3, relativeLayout.getHeight() + i4));
                SignElementPdf fASElement2 = fASElement;
                this.mPage.updateElement(fASElement2, mapRectToPDFCoordinates(new RectF((float) ((int) relativeLayout.getX()), (float) ((int) relativeLayout.getY()), (float) ((int) (relativeLayout.getX() + ((float) view.getWidth()))), (float) ((int) (relativeLayout.getY() + ((float) view.getHeight()))))), 0.0f, 0.0f, viewSign.getStrokeWidth(), 0.0f);
                setMenuPosition(viewSign, this.mElementPropMenu);
            }
        }
    }
    ...
    private void handleDragMove(DragEvent dragEvent) {
        ViewElement.DragEventData dragEventData = (ViewElement.DragEventData) dragEvent.getLocalState();
        ViewElement fASElementViewer = dragEventData.viewer;
        ...
        UtilViewSign.constrainRectXY(rectF, getImageContentRect());
    ...
    private void handleDragEnd(DragEvent dragEvent) {
        if (this.mLastDragPointX != -1.0f || this.mLastDragPointY != -1.0f) {
            ...
            this.mPage.updateElement((SignElementPdf) elementView.getTag(), imageContentRect, 0.0f, f, 0.0f, 0.0f);
    ...
    private void hideDragElement(ViewElement fASElementViewer) {
        removeFocus();
        fASElementViewer.showBorder();
