...
import com.pixelz360.docsign.imagetopdf.creator.RoomDb.adapters.PdfToJpgAdapterRoom;
...
@AndroidEntryPoint
public class PdfToJpgFragment extends Fragment implements ToolbarSettings {
    ...
    private PdfToJpgAdapterRoom pdfToJpgAdapterRoom;
    ...
    public static PdfToJpgFragment newInstance(int layoutId, boolean bannerAdIsLoaded) {
        PdfToJpgFragment fragment = new PdfToJpgFragment();
...
//    }
    ...
    private BroadcastReceiver myReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction() != null && intent.getAction().equals("com.example.MY_CUSTOM_ACTION_PdfToJpgFragment")) {
                receivedStringHomeScreenBackPress = intent.getStringExtra("onBackPressed_PdfToJpgFragment");
//                Toast.makeText(context, "Received in Fragment: " + receivedStringHomeScreenBackPress, Toast.LENGTH_SHORT).show();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        IntentFilter filter = new IntentFilter("com.example.MY_CUSTOM_ACTION_PdfToJpgFragment");
...
//        pdfFileViewModel.getIdRecentPdfFiles().observe(getViewLifecycleOwner(), pdfFileList -> {
            pdfFileViewModel.getPdfToJpjImagesByUserType(AccountsOrGuesHelper.checkAccountOrNot(requireActivity()),"pdf").observe(getViewLifecycleOwner(), pdfFileList -> {
...
//            homeSelectAllButton.setVisibility(GONE);
            ...
            Log.d("checkviewersdf","PdfToJpfFragment updateRecyclerView pdfFileList.isEmpty()");
...
//            homeSelectAllButton.setVisibility(VISIBLE);
            ...
            Log.d("checkviewersdf","PdfToJpfFragment updateRecyclerView pdfFileList is not Empty()");
...
//            homeSelectAllButton.setVisibility(View.VISIBLE);
            ...
            pdfToJpgAdapterRoom = new PdfToJpgAdapterRoom(pdfFileList, requireActivity(),isCheckedListOrGrid, new RvListenerPdfRoom() {
                @Override
                ...
                public void onFavoriteClick(PdfFile pdfFile) {
                    ...
                    if (position != -1) {
                        pdfToJpgAdapterRoom.notifyItemChanged(position);
                    }

                }
            });
            binding.pdfRv.setAdapter(pdfToJpgAdapterRoom);
        }
    }
    ...
    private void filter(String text) {
        ...
        pdfToJpgAdapterRoom.filterList(filteredNames);
                ...
                homeSelectAllButton.setOnClickListener(v -> {
                    ...
                    if (!isFileValieOrNo){
                        ...
                        if (singleItemsSelected) {
                            pdfToJpgAdapterRoom.unselectAllItems();
...
//                                pdfAdapter.selectAllItems("Single All items selected successfully");
                                pdfToJpgAdapterRoom.selectAllItems("Single All items selected successfully");
...
//                pdfFileViewModel.getIdRecentPdfFiles().observe(getViewLifecycleOwner(), pdfFileList -> {
    ...
    public void onPrepareOptionsMenu(@NonNull Menu menu) {
        ...
        try {
            boolean hasSelectedItems = pdfToJpgAdapterRoom.getSelectedItems().size() > 0;
...
//    }
    ...
    private void clearSelection() {
        ...
        if(activity!= null && isAdded()){
            pdfToJpgAdapterRoom.clearSelection();
            requireActivity().invalidateOptionsMenu();
//            binding.allSelctedItemBtn.setBackground(getResources().getDrawable(R.drawable.btn_un_select_bg));
//            binding.allSelctedItemBtn.setTextColor(ContextCompat.getColor(requireActivity(), R.color.select_text_un_selected));
            singleItemsSelected = false;
...
//            homeToolbarTitle.setText(getResources().getString(R.string.docsign_non));
    ...
    private void onDeleteSelectedItems() {
        List<PdfFile> selectedFiles = pdfToJpgAdapterRoom.getSelectedFiles();
...
//            pdfFileViewModel.getIdRecentPdfFiles().observe(this, pdfFileList -> {
                ...
                if (pdfFileList.isEmpty()){
                    Log.d("checkfragmentselected"," PdfToJpgFragment is Empty 1");
                    ...
                    Log.d("checkfragmentselected","PdfToJpgFragment not empty 2");
        ...
        Log.d("checkfragmentselected11","PdfToJpgFragment  onResume");
    ...
    public void onPause() {
        super.onPause();
        Log.d("checkfragmentselected11","PdfToJpgFragment  onPause");
    }
}
